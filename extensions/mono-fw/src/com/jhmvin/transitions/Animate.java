...
public class Animate {
    ...
    public static void fade(Node next, double millis, Runnable callback, Node... current) {
        // disable all node that might be the origin.
        ...
        int origin_size = current.length;

        for (int index = 0; index < origin_size; index++) {
            if ((index + 1) == origin_size) {
                // last
                // run the callback.
                fadeOut(current[index], millis, () -> {
                    // run first callback
                    next.setOpacity(0.0);
                    callback.run();
                    // fade in transition
                ...
                fadeOut(current[index], millis, () -> {
                    // do nothing if this is not the last.
                });
            }
        }

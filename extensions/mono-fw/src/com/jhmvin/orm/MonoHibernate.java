...
public class MonoHibernate {
    ...
    //--------------------------------------------------------------------------
    // bug fix for c3p0 initialization
    /**
     * Since C3P0 will assure deadlock when the database was not found. or the
     * IP Address that has been set was wrong. this method is introduced to
     * check whether that the database is reachable before starting the
     * hibernate instance.
     *
     * @return
     */
    private boolean testForConnection() {
        String url = connectionProvider + "://" + Host + ":" + Port + "/" + DatabaseName;
        Connection con = null;
        try {
            con = DriverManager.getConnection(url, DatabaseUser, DatabasePassword);
            if (!con.isClosed()) {
                // if open
                System.err.println("TEST: CONNECTION TEST SUCCESS");
                return true;
            }
        } catch (SQLException e) {
            System.err.println("TEST: CONNECTION TEST FAILED");
            //e.printStackTrace();
        } finally {
            if (con != null) {
                try {
                    con.close();
                    System.err.println("TEST: CONNECTION CLOSED");
                } catch (Exception e) {
                    // ignore close error
                }
            }
    ...
    public void connect() {
        if (!this.isStarted) {
            ...
            if (!testForConnection()) {
                ...
                throw new RuntimeException("The Server is not reachable Hibernate instance is not started.");
            }

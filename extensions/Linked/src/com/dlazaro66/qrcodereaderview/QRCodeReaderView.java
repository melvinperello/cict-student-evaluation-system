...
/**
 * QRCodeReaderView Class which uses ZXING lib and let you easily integrate a QR
 * decoder view. Take some classes and made some modifications in the original
 * ZXING - Barcode Scanner project.
 * 
 * @author David Lázaro
 */
public class QRCodeReaderView extends SurfaceView implements
	...
	/**
	 * Set QR decoding enabled/disabled. default value is true
	 * 
	 * @param qrDecodingEnabled
	...
	/**
	 * Set Torch enabled/disabled. default value is false
	 * 
	 * @param enabled
	...
	/**
	 * Allows user to specify the camera ID, rather than determine it
	...
	private boolean checkCameraHardware() {
		if (getContext().getPackageManager().hasSystemFeature(
			...
			return true;
		} else if (getContext().getPackageManager().hasSystemFeature(
	...
	private static class DecodeFrameTask extends
			AsyncTask<byte[], Void, Result> {
		...
		public DecodeFrameTask(QRCodeReaderView view,
			...
			viewRef = new WeakReference<QRCodeReaderView>(view);
			hintsRef = new WeakReference<Map<DecodeHintType, Object>>(hints);
		}
		...
		protected Result doInBackground(byte[]... params) {
			...
			final PlanarYUVLuminanceSource source = view.mCameraManager
					.buildLuminanceSource(params[0], view.mPreviewWidth,
							view.mPreviewHeight);
		...
		protected void onPostExecute(Result result) {
			...
			if (view != null && result != null
					&& view.mOnQRCodeReadListener != null) {
				// Transform resultPoints to View coordinates
				final PointF[] transformedPoints = transformToViewCoordinates(
						view, result.getResultPoints());
		 ...
		 * 
		 * This method is needed because coordinates are given in landscape
		 * camera coordinates when device is in portrait mode and different
		...
		private PointF[] transformToViewCoordinates(QRCodeReaderView view,
			...
			Orientation orientation = orientationDegrees == 90
					|| orientationDegrees == 270 ? Orientation.PORTRAIT
			...
			return qrToViewPointTransformer.transform(resultPoints,
					isMirrorCamera, orientation, viewSize, cameraPreviewSize);
		}
	}
}

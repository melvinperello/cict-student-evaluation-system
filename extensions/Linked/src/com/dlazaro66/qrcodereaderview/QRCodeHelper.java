...
public class QRCodeHelper {
	...
	public Bitmap generate() {
		...
		try {
			BitMatrix bitMatrix = new QRCodeWriter().encode(mContent,
			...
			Bitmap bitmap = Bitmap.createBitmap(pixels, 0, mWidth, mWidth,
	...
	private boolean isInRect(int x, int y) {
		return ((x > mMargin * 8 && x < mWidth - mMargin * 8) && (y > mMargin * 8 && y < mHeight
				- mMargin * 8));
	}
	...
	private Bitmap addLogo(@NonNull Bitmap qrCodeBitmap, @NonNull Bitmap logo) {
		...
		Bitmap blankBitmap = Bitmap.createBitmap(qrCodeWidth, qrCodeHeight,
		...
		while ((logoWidth / scaleSize) > (qrCodeWidth / 5)
				|| (logoHeight / scaleSize) > (qrCodeHeight / 5)) {
			scaleSize *= 2;
		}
		float sx = 1.0f / scaleSize;
		canvas.scale(sx, sx, qrCodeWidth / 2, qrCodeHeight / 2);
		canvas.drawBitmap(logo, (qrCodeWidth - logoWidth) / 2,
				(qrCodeHeight - logoHeight) / 2, null);
		canvas.restore();
		return blankBitmap;
	}
}
...
public class ConfigurationManager {
    ...
    public final static File CONNECTION_PROP = new File("configuration/linked_tv.properties");

    //--------------------------------------------------------------------------
    public final static String PROP_HOST_IP = "server";
    ...
    private static boolean checkConfigurationFile() {
        if (!CONFIGURATION_DIR.exists()) {
            try {
                ...
                if (folder_created) {
                    boolean file_created = CONNECTION_PROP.createNewFile();
                    return file_created;
                }
            } catch (Exception e) {
                return false;
    ...
    public static void checkConfiguration() {
        ...
        if (config_ready) {
            Properties property = PropertyFile.getPropertyFile(CONNECTION_PROP.getAbsolutePath());
            String val = property.getProperty(PROP_HOST_IP);
            if (val == null) {
                PropertyFile.writePropertyFile(CONNECTION_PROP.getAbsolutePath(), PROP_HOST_IP, "127.0.0.1");
            }
        }
    }
}
